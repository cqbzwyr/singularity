...
class MultipleBuildDialog(dialog.FocusDialog, BuildDialog):
    ...
    def on_field_change(self, value):
        ...
        if new_field_text != orig_field_text:
            ...
            self.count_field.cursor_pos = min(self.count_field.cursor_pos, len(new_field_text))
...
class BaseScreen(dialog.Dialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.contents_frame = \
            widget.BorderedWidget(self, (0, .09), (.50, .45),
                                  anchor=constants.TOP_LEFT,
                                  background_color="pane_background",
                                  borders=range(6))

        self.auto_build_best = \
            button.FunctionButton(self, (0, 0.54), (0.08, 0.08),
                                  autotranslate=True,
    ...
    def _any_item_slots_empty(self):
        ...
        for item_spec in item_list:
            ...
            if existing_item is not None:
                continue
            if not item_spec.available() or not item_spec.buildable_in(self.base.location):
                continue
            return True
        return False
